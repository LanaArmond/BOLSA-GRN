...
class Plotter:
    ...
    @staticmethod
    def plot_scatter_comparison(results, df, t_eval, t, solvers, labels):
    ...
    @staticmethod
    def plot_comparison(results, t, df, solvers, labels):
    ...
    @staticmethod
    def plot_ind(ind, solver='RK45', comparison=True):
        model = ind.model
        solvers = [solver]
        results = {}
        results[solver] = ind.solve_ivp(solver=solver)

        Plotter.plot_scatter_comparison(results, model.df, model.t_eval, model.df['t'], solvers, model.labels)
                
        # Se comparison=True, plota a comparação usando Plotter
        if comparison:
            Plotter.plot_comparison(results=results, t=model.t_eval, df=model.df, solvers=solvers, labels=model.labels)